    ...
    <div className="min-h-screen bg-neutral-50 font-sans text-neutral-900 scroll-smooth overflow-x-hidden">
      <nav className="fixed w-full z-40 bg-white/90 backdrop-blur-md border-b border-neutral-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <button className="md:hidden p-2" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>{mobileMenuOpen ? <X /> : <MenuIcon />}</button>
          </div>
        </div>
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="md:hidden bg-white border-b border-neutral-200 overflow-hidden">
      ...
      <section id="inicio" className="relative h-screen flex items-center overflow-hidden">
        <div className="absolute inset-0 z-0 bg-neutral-100">
          <motion.div animate={{ scale: [1, 1.1, 1], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 10, repeat: Infinity }} className="absolute top-20 right-[-10%] w-[600px] h-[600px] bg-neutral-200 rounded-full blur-[120px]" />
          <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 15, repeat: Infinity, delay: 2 }} className="absolute bottom-20 left-[-5%] w-[500px] h-[500px] bg-neutral-300 rounded-full blur-[100px]" />
        </div>
        <div className="container mx-auto px-6 relative z-10 grid md:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, ease: "easeOut" }} className="space-y-8">
            <div className="inline-block px-4 py-1.5 bg-white border border-neutral-200 rounded-full text-[10px] uppercase tracking-[0.3em] font-bold text-neutral-600 shadow-sm">Sabor que trasciende</div>
            <h1 className="text-6xl md:text-8xl font-extrabold tracking-tighter leading-[0.9]">D'ARELI <br /><span className="text-neutral-400 font-light italic">BANQUETERÍA</span></h1>
            <p className="text-xl text-neutral-600 max-w-md leading-relaxed font-light">Donde la alta cocina se encuentra con la distinción. Creamos banquetes memorables con un sello de sofisticación único.</p>
            <div className="flex flex-wrap gap-4 pt-6">
              <motion.a whileHover={{ x: 5 }} href="#servicios" className="px-8 py-4 bg-black text-white rounded-none hover:bg-neutral-800 transition-all flex items-center gap-3 group text-xs uppercase tracking-widest font-bold">Explorar Menús <ArrowRight className="w-4 h-4" /></motion.a>
              <a href="#contacto" className="px-8 py-4 border border-neutral-300 bg-white/50 backdrop-blur rounded-none hover:bg-neutral-100 transition-all text-xs uppercase tracking-widest font-bold">Cotizar Evento</a>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 1.2, ease: "easeOut", delay: 0.2 }} className="hidden md:block relative group">
            <motion.div animate={{ x: [24, 12, 24], y: [24, 12, 24] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} className="absolute -inset-4 border border-neutral-200 z-0" />
            <div className="aspect-[4/5] relative bg-neutral-200 shadow-2xl overflow-hidden z-10">
              <img src="https://images.unsplash.com/photo-1559339352-11d035aa65de?auto=format&fit=crop&q=80&w=800" alt="Alta Gastronomía Gourmet" className="w-full h-full object-cover grayscale-[0.2] group-hover:grayscale-0 transition-all duration-1000 group-hover:scale-105" onError={(e) => { const t = e.currentTarget; if (t) t.src = 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&q=80&w=800'; }} />
              <div className="absolute bottom-0 left-0 right-0 p-10 bg-gradient-to-t from-black/90 via-black/40 to-transparent text-white">
                <motion.div initial={{ width: 0 }} whileInView={{ width: 48 }} transition={{ delay: 0.5, duration: 0.8 }} className="h-1 bg-white mb-6" />
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="flex justify-center mb-16 border-b border-neutral-100">
            {['banqueteria', 'almuerzos'].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`relative px-10 py-5 text-xs font-bold uppercase tracking-[0.3em] transition-all ${activeTab === tab ? 'text-black' : 'text-neutral-400 hover:text-neutral-600'}`}>
                {tab === 'banqueteria' ? 'Banquetería Gala' : 'Almuerzos & Matrimonios'}
                {activeTab === tab && <motion.div layoutId="activeTabUnderline" className="absolute bottom-0 left-0 right-0 h-1 bg-black" />}
              </button>
            ))}
          </div>
          <AnimatePresence mode="wait">
            {activeTab === 'banqueteria' ? (
              <motion.div key="gala-content" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="max-w-5xl mx-auto bg-neutral-50 border border-neutral-100 p-8 md:p-20 shadow-2xl relative">
                <div className="absolute top-0 right-0 w-80 h-80 bg-neutral-200/30 rounded-full blur-[100px] -mr-40 -mt-40"></div>
                <div className="text-center space-y-3 mb-16">
                  <div className="flex justify-center mb-8"><motion.div animate={{ rotate: [0, 10, 0] }} transition={{ duration: 4, repeat: Infinity }} className="w-14 h-14 flex items-center justify-center border-2 border-black rounded-full"><Utensils className="w-6 h-6" /></motion.div></div>
                ...
                <div className="mt-20 pt-10 border-t border-neutral-200 flex flex-col md:flex-row justify-between items-center gap-8">
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="px-12 py-5 bg-black text-white text-[10px] uppercase tracking-[0.3em] font-black hover:bg-neutral-800 transition-all shadow-xl">Solicitar Dossier Completo</motion.button>
                </div>
              </motion.div>
            ) : (
              <motion.div key="almuerzos-content" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {groupMenus.map((menu, idx) => (
                  <motion.div key={menu.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1, duration: 0.6 }} whileHover={{ y: -12 }} className="relative flex flex-col h-full bg-white border border-neutral-100 shadow-[0_15px_40px_-15px_rgba(0,0,0,0.1)] group overflow-hidden">
                    ...
                    <div className="p-10 flex flex-col flex-grow items-center text-center">
                      ...
                      <div className="w-full pt-8 border-t border-neutral-50">
                        ...
                        <button onClick={() => setSelectedMenu(menu)} className={`w-full py-4 text-[10px] uppercase tracking-[0.3em] font-black border transition-all ${menu.accent} hover:bg-neutral-50`}>Ver Detalles</button>
                      </div>
                    </div>
                  </motion.div>
                ))}
      ...
      <section id="quienes-somos" className="py-28 bg-neutral-900 text-white overflow-hidden relative">
        <div className="container mx-auto px-6 grid md:grid-cols-2 gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 60 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1, ease: "easeOut" }} className="relative">
            <div className="aspect-square bg-neutral-800 relative overflow-hidden group">
              <img src="https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?auto=format&fit=crop&q=80&w=800" alt="Chef trabajando en detalle" className="w-full h-full object-cover opacity-40 group-hover:opacity-100 transition-opacity duration-1000" />
          ...
          <motion.div {...fadeInUp} className="max-w-6xl mx-auto bg-white shadow-[0_50px_100px_-20px_rgba(0,0,0,0.12)] overflow-hidden flex flex-col md:flex-row border border-neutral-100">
            ...
            <div className="md:w-3/5 p-16">
              <form className="space-y-10">
                ...
                <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} type="button" className="w-full md:w-auto px-16 py-5 bg-black text-white text-[11px] uppercase tracking-[0.4em] font-black hover:bg-neutral-800 transition-all shadow-lg">Enviar Solicitud</motion.button>
              </form>
            </div>
          </motion.div>
        </div>
        ...
        {selectedMenu && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-xl" onClick={() => setSelectedMenu(null)}>
            <motion.div initial={{ scale: 0.9, y: 30, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 30, opacity: 0 }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="bg-white w-full max-w-3xl shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <button onClick={() => setSelectedMenu(null)} className="absolute top-6 right-6 p-2.5 hover:bg-neutral-100 rounded-full transition-colors z-10"><X className="w-6 h-6" /></button>
              <div className="p-10 md:p-20 text-center">
                ...
                <div className="mt-16 flex flex-col md:flex-row justify-between items-center gap-10 pt-10 border-t border-neutral-100">
                  ...
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} type="button" className="w-full md:w-auto px-14 py-5 bg-black text-white text-[11px] uppercase tracking-[0.4em] font-black shadow-xl">Solicitar Presupuesto</motion.button>
                </div>
              </div>
            </motion.div>
          </motion.div>
